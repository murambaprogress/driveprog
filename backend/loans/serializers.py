...
class LoanApplicationDocumentSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ['id', 'document_type', 'title', 'file', 'description', 'uploaded_at', 'is_analyzed', 'ai_analysis_result']
        read_only_fields = ['id', 'uploaded_at', 'is_analyzed', 'ai_analysis_result']
        ...
        try:
            ...
            for field in photo_fields:
                if getattr(obj, field):
                    document_count += 1
                    document_types.append(field.replace('photo_', '').replace('_', ' ').title())
            
            # Count related documents
            if hasattr(obj, 'documents'):
                related_docs = obj.documents.all()
                document_count += related_docs.count()
                document_types.extend([doc.document_type for doc in related_docs])
...
class LoanApplicationUpdateSerializer(LoanApplicationDataMixin, serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'credit_score', 'accept_terms', 'signature', 'is_draft', 'status' # Added status
        ]
