...
const Step3Vehicle = ({ onNext, onBack }) => {
  ...
  useEffect(() => {
    if (formData?.vehicle && Object.keys(formData.vehicle).length > 0) {
      console.log('Resetting Vehicle form with saved data:', formData.vehicle);
      Object.entries(formData.vehicle).forEach(([key, value]) => {
        setValue(key, value);
      });
    }
  }, [loanId, setValue, formData?.vehicle]);
      ...
      <MDBox component="form" role="form" onSubmit={handleSubmit(onSubmit)}>
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} md={6}>
            ...
            <VhoozhtInput
              {...register('estimatedCarValue', { 
                required: 'Estimated car value is required',
                validate: (value) => {
                  const carValue = parseFloat(value);
                  const loanAmount = parseFloat(formData?.personal?.loanAmount) || 0;
                  
                  if (loanAmount > 0 && carValue < loanAmount * 1.8) {
                    return `Estimated car value must be at least $${(loanAmount * 1.8).toFixed(2)} (1.8x the loan amount of $${loanAmount.toFixed(2)})`;
                  }
                  return true;
                }
              })}
